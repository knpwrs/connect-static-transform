...
Optionally, the following keys may be specified:
  ...
  maxage: Sets the maximum number of seconds a client and server should cache the output for. Defaults to one year on the client. When cache expires, transformation runs again.
...
function createMiddleware(options) {
  ...
  return function staticTransform(req, res, next) {
    ...
    // Check if object has expired and clear cache
    if (options.cache && cache[path]) {
      if(cache[path].cacheExpires){
        var now = new Date();
        // For some reason it's sometimes nessesary to re-cast cacheExpires to a Date-object 
        var cacheExpires = new Date(cache[path].cacheExpires); 
        if (now > cacheExpires) {
    ...
    if (options.cache && cache[path]) {
      ...
      fs.stat(path, function (err, stat) {
        ...
        function send(out, headers) {
          ...
          if (options.cache) {
            var expireHeader = new Date();
            expireHeader.setYear(expireHeader.getFullYear() + 1);
            var now = new Date();
            var cacheExpires = undefined;
            if (options.maxage) cacheExpires = new Date(now.getTime() + (1000 * options.maxage));
            headers['Expires'] = options.expires || expireHeader.toUTCString();
            headers['Cache-Control'] = 'public, max-age=' + (options.maxage || 31536000/*One year in seconds*/);
            headers['Last-Modified'] = stat.mtime.toUTCString();
            cache[path] = {out: out, headers: headers, cacheExpires: cacheExpires};
          }
